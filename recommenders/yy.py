...
def collab_model(product_list,top_n=10):
    

    indices = pd.Series(products_df['productId'])
    product_ids = pred_products(product_list)
    df_init_users = ratings_df[ratings_df['userId']==product_ids[1]]
    ...
    for i in product_ids :
        ...
        df_init_users=le.fit_transform(df_init_users)
    
    #Getting the cosine similarity matrix
    cosine_sim = cosine_similarity(np.array(df_init_users.to_), np.array(df_init_users))
